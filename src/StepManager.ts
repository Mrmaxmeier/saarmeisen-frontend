...
export class StepManager {
	...
	hasPrev(): boolean {
		return this.state.currentStepIndex > 0 && this.state.currentStepIndex <= this.undo.length;
	}
	...
	next(): GameState {
		...
		if (this.state.currentStepIndex >= this.undo.length) {
			...
			let undo = step.fields.map(
				({ x, y }) => this.state.fields.find((other) => other.x === x && other.y === y)!
			);
			this.undo.push(undo);
		}
		...
		this.state = { fields, standings, currentStepIndex: this.state.currentStepIndex + 1 };
		return this.state;
	}
	...
	prev(): GameState {
		...
		let fields = this.applyFieldChange(this.state.fields, this.undo[currentStepIndex]);
